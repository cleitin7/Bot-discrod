...
TOKEN = os.getenv("TOKEN")  # 🔐 Pegando o token de forma segura
GUILD_ID = 1198358681130111148
...
@bot.event
async def on_ready():
    await bot.change_presence(status=discord.Status.invisible)  # BOT OFFLINE
    print(f"🤖 Bot logado como {bot.user}")
    try:
        synced = await bot.tree.sync(guild=discord.Object(id=GUILD_ID))  # Sincroniza só no servidor
